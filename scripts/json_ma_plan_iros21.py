...
def parse():
    ...
    parser.add_argument("-t", "--temporal_actions", nargs='+', type=str, default=[], 
...
#################################################################
def action_json(action, temporal_actions=[]):
    ...
    if action.sig[0] in temporal_actions: 
        action_json['type'] = "durative"
    else:
        action_json['type'] = "primitive"
...
#################################################################
def json_ma_plan(policy, agents=[], temporal_actions=[], verbose=False):
    '''
        ...
        if len(subplans) == 1: # there is only one parallel subplan
            ...
            if len(subplans[0]) == 1: # there is only one step in this subplan
                ...
                if len(subplans[0][key]) == 1: # there is only one action in this step
                    ...
                    action_descriptions_json[action_ref] = action_json(subplans[0][key][0], temporal_actions)
                # ----------------------------------------------------
                    ...
                    # there is a non-progressive action, so turn to concurrent execution
                    if set(temporal_actions) & set([action.sig[0] for action in subplans[0][key]]):
                        plan_json[subplan_ref] = OrderedDict({'list':[],'ordering':'concurrent'})
                    else:
                    ...
                    for i, action in enumerate(subplans[0][key]):
                        ...
                        # add ref and its description into the action_descriptions_json
                        action_descriptions_json[action_ref] = action_json(action, temporal_actions)
            # ----------------------------------------------------
                ...
                for i, (k, step) in enumerate(subplans[0].items()):
                    ...
                    if len(step) == 1: # there is only one action in this step
                        ...
                        plan_json[subplan_ref]['list'].append(action_ref)
                        # add ref and its description into the action_descriptions_json
                        action_descriptions_json[action_ref] = action_json(step[0], temporal_actions)
                    # ----------------------------------------------------
                    else: # there are more actions in this step
                        # make a reference to subplan (n)
                        ...
                        # there is a non-progressive action, so turn to concurrent execution
                        if set(temporal_actions) & set([action.sig[0] for action in step]):
                            plan_json[subsubplan_ref] = OrderedDict({'list':[],'ordering':'concurrent'})
                        else:
            ...
            for i, subplan in enumerate(subplans):
                ...
                if len(subplan) == 1: # there is one step in this subplan
                    ...
                    if len(list(subplan.values())[0]) == 1: # there is only one action in this step
                        ...
                        plan_json[subplan_ref]['list'].append(action_ref)
                        # add ref and its description into the action_descriptions_json
                        action_descriptions_json[action_ref] = action_json(list(subplan.values())[0][0], temporal_actions)
                    # ----------------------------------------------------
                    else: # there are more actions in this step
                        # make a reference to subplan (n)
                        subsubplan_ref = 'subplan_{}_{}'.format(n,i)
                        plan_json[subplan_ref]['list'].append(subsubplan_ref)
                        # there is a non-progressive action, so turn to concurrent execution
                        if set(temporal_actions) & set([action.sig[0] for action in list(subplan.values())[0]]):
                            plan_json[subsubplan_ref] = OrderedDict({'list':[],'ordering':'concurrent'})
                        else:
                            plan_json[subsubplan_ref] = OrderedDict({'list':[],'ordering':'partial'})
                ...
                # ----------------------------------------------------
                    ...
                    for j, (k, step) in enumerate(subplan.items()):
                        ...
                        # ----------------------------------------------------
                            ...
                            # there is a non-progressive action, so turn to concurrent execution
                            if set(temporal_actions) & set([action.sig[0] for action in step]):
                                plan_json[subsubsubplan_ref] = OrderedDict({'list':[],'ordering':'concurrent'})
                            else:
...
def make_plan(domain, problem, planners=['optic-clp'], agents=[], temporal_actions=[], rank=False, verbose=False):
    ...
    policy = planner.Planner(domain=domain, problem=problem, planners=planners, rank=rank, verbose=verbose)
    ...
    plan_json_file, actions_json_file = json_ma_plan(policy, agents, temporal_actions)

    print(color.fg_yellow('-- plan_json_file:') + plan_json_file)
    print(color.fg_yellow('-- actions_json_file:') + actions_json_file)
    print(color.fg_yellow('-- graphical plan_json_file:') + plan_json_file + '.dot')
    ...
    dot_file = dot_plan.gen_dot_plan(plan=plan, domain_file=domain, problem_file=problem)
    print(color.fg_yellow('-- plan in dot file: ') + dot_file)
    # transform the plan into a parallel plan
    dot_file, tred_dot_file = dot_ma_plan.parallel_plan(policy, verbose=verbose)
...
if __name__ == '__main__':
    ...
    #############################
    # get possible concurrent and joint executions
    single_executions, joint_executions = concurrent_executions(policy, plan, args.agents)
    ...
    #############################
    # refine and separate the concurrent executions into concurrent clusters
    main_list = concurrent_subplans(policy, plan, args.agents)
    ...
    #############################
    # convert the plan inti a concurrent plan in json files
    plan_json_file, actions_json_file = json_ma_plan(policy, args.agents, args.temporal_actions)
    ...
    if args.dot:
        ...
        print(color.fg_yellow('-- precedence graph: ') + dot_file)
        print(color.fg_yellow('-- transitive reduction: ') + tred_dot_file)
        os.system('xdot %s &' % tred_dot_file)
